...
func NewApp(ctx context.Context, cfg *config.Config, log *slog.Logger) (*App, error) {
	...
	agentService := agentusecase.NewService(agentRepo, agentProfileRepo, flowVersionRepo, runRepo, runEventRepo, nodeLogRepo, runStepRepo, memoryRepo, memoryWriteLogRepo, toolDefinitionRepo, toolInvocationRepo, providerRepo, messageRepo, retrievalService, chatClient, secretBox)
