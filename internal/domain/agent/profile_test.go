...
func TestProfileDefaultCallAgentIDsSlice(t *testing.T) {
	p := &Profile{DefaultCallAgentIDs: json.RawMessage(`[100, 200, 300]`)}
	ids := p.DefaultCallAgentIDsSlice()
...
func TestProfileDefaultCallAgentIDsEmpty(t *testing.T) {
	p := &Profile{DefaultCallAgentIDs: json.RawMessage(`[]`)}
	ids := p.DefaultCallAgentIDsSlice()
	if len(ids) != 0 {
...
func TestProfileNormalizeJSONNull(t *testing.T) {
	p := &Profile{
		...
		DefaultCallAgentIDs: json.RawMessage("null"),
	...
	if string(p.DefaultCallAgentIDs) != "[]" {
		t.Fatalf("expected [], got %s", p.DefaultCallAgentIDs)
	}
...
func TestProfileNormalizeJSONPreservesValid(t *testing.T) {
	p := &Profile{
		...
		DefaultCallAgentIDs: json.RawMessage(`[3]`),
	...
	if string(p.DefaultCallAgentIDs) != `[3]` {
		t.Fatalf("expected [3], got %s", p.DefaultCallAgentIDs)
	}
...
func TestProfileTableName(t *testing.T) {
	p := Profile{}
	if p.TableName() != "agent_profiles" {
		t.Fatalf("expected agent_profiles, got %s", p.TableName())
	}
}
