...
type Message struct {
	...
	// TranscriptEntryID is assigned only to realtime runtime rows. A nullable
	// value lets ordinary user/fork messages keep their existing semantics while
	// the database enforces exactly-once writes for retried transcript entries.
	TranscriptEntryID *string         `json:"transcript_entry_id,omitempty" gorm:"column:transcript_entry_id"`
