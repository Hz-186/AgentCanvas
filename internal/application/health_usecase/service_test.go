...
func (*backendFake) ReflectionSystem(context.Context) (map[string]any, error) {
	return map[string]any{"component": "reflection_system"}, nil
}
...
func TestServiceDelegatesHealthChecks(t *testing.T) {
	...
	if snapshot, err := service.ReflectionSystem(context.Background()); err != nil || snapshot["component"] != "reflection_system" {
		t.Fatalf("reflection snapshot was not delegated: snapshot=%v error=%v", snapshot, err)
	}
