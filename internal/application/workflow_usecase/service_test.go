...
func TestUpdateAgentProfileValidatesLimits(t *testing.T) {
	...
	legacyMode := "reflect"
	if _, err := service.UpdateWorkflowProfile(context.Background(), 1, 20, UpdateWorkflowProfileRequest{Mode: &legacyMode}); err == nil {
		t.Fatal("expected legacy reflect mode to be rejected")
	}
}
