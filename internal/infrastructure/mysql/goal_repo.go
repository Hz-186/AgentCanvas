...
func (r *GoalRepository) ClaimContinuation(ctx context.Context, ownerID, conversationID int64, goalID string) (bool, error) {
	if ownerID <= 0 || conversationID <= 0 || goalID == "" {
		return false, goal.ErrConflict
	}
	claimed := false
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var row goal.ContinuationClaim
		err := tx.Where("owner_id = ? AND conversation_id = ?", ownerID, conversationID).First(&row).Error
		now := time.Now().UTC()
		if errors.Is(err, gorm.ErrRecordNotFound) {
			row = goal.ContinuationClaim{BaseModel: domain.BaseModel{OwnerID: ownerID}, ConversationID: conversationID, GoalID: goalID, ClaimedAt: now}
		...
		if row.ClaimedAt.After(now.Add(-60 * time.Second)) {
			return nil
		}
		result := tx.Model(&goal.ContinuationClaim{}).Where("id = ? AND claimed_at = ?", row.ID, row.ClaimedAt).Updates(map[string]any{"goal_id": goalID, "claimed_at": now, "updated_at": now})
		if result.Error != nil {
			return result.Error
		}
...
func (r *GoalRepository) ReleaseContinuation(ctx context.Context, ownerID, conversationID int64, goalID string) error {
	return r.db.WithContext(ctx).Where("owner_id = ? AND conversation_id = ? AND goal_id = ?", ownerID, conversationID, goalID).Delete(&goal.ContinuationClaim{}).Error
}
