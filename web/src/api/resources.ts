...
export const workflowApi = {
  ...
  publishRuleSet: (id: number, ruleSetId: number, expectedRevision: number, idempotencyKey?: string) => api.post<RuleCompileJob>(`/workflows/${id}/rule-sets/${ruleSetId}/publish`, { expected_revision: expectedRevision }, idempotencyKey ? { headers: { 'Idempotency-Key': idempotencyKey } } : undefined),
  reviewRuleSet: (id: number, ruleSetId: number, body: { expected_revision: number; decisions: Array<{ edge_id: number; decision: 'accepted' | 'rejected' }> }) => api.post<WorkflowRuleSet>(`/workflows/${id}/rule-sets/${ruleSetId}/review`, body),
  rollbackRuleSet: (id: number, ruleSetId: number) => api.post<WorkflowRuleSet>(`/workflows/${id}/rule-sets/${ruleSetId}/rollback`),
  getRuleCompileJob: (id: number, jobId: number) => api.get<RuleCompileJob>(`/workflows/${id}/rule-compile-jobs/${jobId}`),
