...
describe('parseRunStreamEvent', () => {
  it.each([
    ...
    ['todo.updated', { explanation: 'working', plan: [{ step: 'inspect', status: 'in_progress' }] }],
    ['goal.updated', { goal: { conversation_id: 7, goal_id: 'g1', objective: 'ship', status: 'active', tokens_used: 1, time_used_seconds: 0 } }],
  ...
  it('rejects unknown kinds and incomplete payloads', () => {
    ...
    expect(() => parseRunStreamEvent({ ...base, kind: 'todo.updated', data: { plan: [{ step: 'inspect', status: 'unknown' }] } })).toThrow('status');
    expect(() => parseRunStreamEvent({ ...base, kind: 'goal.updated', data: { goal: { status: 'unknown' } } })).toThrow('status');
