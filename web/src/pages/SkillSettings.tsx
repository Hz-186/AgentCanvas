...
export function SkillSettings() {
  ...
  const [sourceType, setSourceType] = useState<"inline" | "local_path">(
    "inline",
  );
    ...
    try {
      ...
      setSkills(
        page.items.map((item) => ({
          ...
          skill_type:
            item.resource_type === "bundle" ? "bundle" : "instruction",
          source_type: "inline",
          entry_file: "SKILL.md",
      ...
      setError(friendlyErrorMessage(err, "加载 Skill 失败"));
    }
  }
  useEffect(() => {
    void load();
  ...
  async function save(event: FormEvent) {
    ...
    const body = {
      ...
      content_markdown: sourceType === "inline" ? content : undefined,
      bundle_path: sourceType === "local_path" ? bundlePath : undefined,
      tags: tags
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean),
    ...
    try {
      if (editingId) await settingsApi.skills.update(editingId, body);
      else await settingsApi.skills.create(body);
      setOpen(false);
      setMessage(editingId ? "Skill 已更新" : "Skill 已创建");
      await load();
    } catch (err) {
      setError(friendlyErrorMessage(err, "保存 Skill 失败"));
    }
  }
  ...
  async function edit(summary: Skill) {
    try {
      ...
      setBundlePath(item.bundle_path ?? "");
      setTags(Array.isArray(item.tags_json) ? item.tags_json.join(", ") : "");
      setOpen(true);
    } catch (err) {
  ...
  return (
    <>
      ...
      <Panel
        className="management-panel section-skills"
        title="Skills"
        eyebrow="Capability"
        action={
      ...
      >
        <div className="stack">
          {skills.length === 0 ? (
            <EmptyState
              title="还没有 Skill"
              description="新增 Skill 后，Agent 可以在运行时按需加载这些说明。"
            />
          ) : (
            skills.map((item) => (
              <article className="card" key={item.id}>
                ...
                <p className="muted truncate">
                  {item.source_type} · {item.entry_file}
                </p>
                <p className="muted truncate">
                ...
                {item.last_validation_error ? (
                  <p className="error-text clamp-2">
                    {item.last_validation_error}
                  </p>
                ) : null}
                <div className="row-wrap">
                  <Button onClick={() => void edit(item)}>
                    <Pencil size={15} />
                    编辑
                  </Button>
                  <Button onClick={() => void validate(item.id)}>
                    <Zap size={16} />
                    校验
                  </Button>
                  <IconButton
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button form="create-skill-form" tone="primary">
              保存
            </Button>
          </>
        }
        ...
        >
          ...
          <Field label="Source Type">
            <Select
              value={sourceType}
              onChange={(event) =>
                setSourceType(event.target.value as "inline" | "local_path")
              }
            >
          ...
          <Field label="Tags" hint="多个标签用英文逗号分隔">
            <TextInput
              ...
              placeholder="review, repo, safety"
            />
          </Field>
        </form>
      </Modal>
