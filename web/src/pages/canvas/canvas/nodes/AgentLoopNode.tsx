...
export function AgentLoopNode({ data, selected }: NodeProps<CanvasNode>) {
  ...
  return (
    <NodeWrapper data={data} selected={selected} className="agent-loop-node" toolsHandle subAgentHandle>
      ...
      <div className="agent-node-metrics">
        <span title="HTTP tools"><Wrench size={12} />{toolCount}</span>
        <span title="Knowledge tools"><Database size={12} />{kbCount}</span>
        <span title="MCP servers"><PlugZap size={12} />{mcpCount}</span>
        <span title="Sub agents"><Bot size={12} />{subAgentCount}</span>
      </div>
