...
export function ProjectsPage() {
  ...
  async function create(event: FormEvent) {
    ...
    try { await projectApi.create({ name: name.trim(), repository_root: path.trim(), initialize_git: true }); setOpen(false); setName(''); setPath(''); await reload(); }
  ...
  async function updateProject(event: FormEvent) {
    ...
    try {
      await projectApi.update(editProject.id, { name: editName.trim(), description: editDescription.trim() });
  ...
  async function addFolder(event: FormEvent) {
    ...
    setBusy(true); setError('');
    try {
      await projectApi.addFolder(folderProject.id, { path: folderPath.trim(), label: folderLabel.trim(), is_repository_root: folderPrimary });
  ...
  async function copyMergeCommand(project: Project, tree: GitWorktree) {
    ...
    const command = `git -C ${quote(project.repository_root)} merge --no-ff ${quote(tree.branch)}`;
    try {
  ...
  return <div className="page">
    ...
    <Modal open={editProject != null} title={`Edit Project · ${editProject?.name ?? ''}`} onClose={() => setEditProject(null)}><form className="stack" onSubmit={(event) => void updateProject(event)}><Field label="Project name"><TextInput value={editName} onChange={(event) => setEditName(event.target.value)} /></Field><Field label="Description"><TextInput value={editDescription} onChange={(event) => setEditDescription(event.target.value)} /></Field><Button tone="primary" type="submit" disabled={busy || !editName.trim()}>Save Project</Button></form></Modal>
    <Modal open={folderProject != null} title={`Project Folders · ${folderProject?.name ?? ''}`} onClose={() => setFolderProject(null)}><div className="stack">
      {(folderProject?.folders ?? []).map((folder) => <div className="card-title" key={folder.id}><div><strong>{folder.label || 'Folder'}{folder.is_repository_root ? ' · Primary' : ''}</strong><p className="muted">{folder.path}</p></div><Button tone="danger" disabled={folder.is_repository_root} onClick={() => void removeFolder(folder.project_id, folder.id)}>Remove</Button></div>)}
