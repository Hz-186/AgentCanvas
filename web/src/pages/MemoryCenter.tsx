...
const memoryTypes = [
  { value: 'profile', label: '个人画像（偏好与设定）' },
  { value: 'episodic', label: '事件记忆（具体经历）' },
  { value: 'task', label: '任务记忆（目标与待办）' },
  { value: 'archival', label: '归档记忆（长期事实）' },
];
  ...
  return (
    <div className="page memory-page">
      ...
      <div className="row-wrap" role="tablist" aria-label="Memory Center views">{([
        ['active', `有效记忆 ${memories.filter((item) => (item.status || 'active') === 'active' && !item.has_conflict).length}`],
        ['pending', `待审核 ${candidates.length}`], ['conflicts', `冲突 ${memories.filter((item) => item.has_conflict).length}`],
